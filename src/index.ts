...
async function headaiPost<T>(apiKey: string, endpoint: string, data: Record<string, unknown>, timeoutMs = 60000): Promise<T> {
...
Server-enforced preview gate: first call returns preview+hash, second call starts the build. Returns async status_url for polling via headai_check_build_status.`,
  ...
  async (params, extra) => {
    try {
      ...
      // but Megatron's v2 API DOES support city filtering for curriculum
      // (confirmed by estimate_size returning 895 for curriculum+city=Jyväskylä).
      // The override was the bug — it silently widened to national data.
      {
  ...
  async (params, extra) => {
    try {
      ...
      const normalizedSkills = [...new Set(
        params.skills.flatMap((s: string) => {
          const spaced = s.replace(/_/g, " ");
          return spaced !== s ? [s, spaced] : [s];
        })
      )];
      ...
      if (params.interests) {
        data.interests = [...new Set(
          params.interests.flatMap((s: string) => {
            const spaced = s.replace(/_/g, " ");
            return spaced !== s ? [s, spaced] : [s];
          })
        )];
      ...
      if (params.country_limit) {
        ...
        // Job searches return empty without country_limit — default to Finland
        data.country_limit = ["fi"];
      }
...
Always describe results by algorithm purpose (e.g. "gap analysis", "emerging trends") — never by numeric code. Glossary: ego1 = skill cluster, bridge = cross-field connector, degree = connectivity, weight 5 = highly specialized.`,
  ...
  async (params) => {
    try {
      ...
      const timeoutPromise = new Promise<null>((resolve) => setTimeout(() => resolve(null), ANALYST_FIRST_TRY_MS));
      const resultOrNull = await Promise.race([
        qaGet<string>("run-junior", queryParams).catch(() => null),
        timeoutPromise,
      ]);
      ...
      // Didn't finish in time — QA server is still computing but will cache the result
      return {
        content: [{
          type: "text",
          text: `Report ${params.report} is still being generated by the analysis engine (large graphs can take 1-3 minutes). ` +
                `The server caches results — call this tool again in 30-60 seconds with the same parameters and it should return instantly.\n\n` +
...
Phase 2 return: status "stored", twin_key, final_skill_count, graph_url, visualizer_url`,
    inputSchema: {
      ...
      legend: z.string().optional().describe("Optional visualization label for the graph. Headai convention: legend is the chart label, title is the canonical name. When title is empty, legend acts as the title — usually identical in practice."),
